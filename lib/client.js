	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				// at the elapsed fraction. The gray deliberately reuses the quota
				// bar's track token so the two stacked grays read as one shade.
				".ocg-timetrack{box-sizing:border-box;flex:none;display:flex;width:100%;height:4px;border-radius:2px;overflow:hidden;background:var(--dsw-alias-border-l2)}",
				".ocg-timeelapsed{height:100%;flex:none;background:var(--dsw-alias-border-l2);transition:width .4s ease}",
				".ocg-timeremain{height:100%;flex:1 1 auto;min-width:0;background:var(--dsw-alias-brand-primary);transition:width .4s ease}",
